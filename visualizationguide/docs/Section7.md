...
This section outlines the steps to visualize black hole with accretion disk data from COCAL using VisIt. Follow this sequence to ensure a consistent and successful plotting process.
...
Ensure the following key COCAL output files are present: **bhflu_3D.las, bhgrids_3D.las**, and any other relevant `.las` files

You can either softlink or copy them:

    ln -s /path/to/bhflu_3D.las .
    ln -s /path/to/bhgrids_3D.las .
    ln -s /path/to/bhtpar.dat .

### Set Up VisIt Color Table
Copy the color table <b>bhbhdisk_pink.ct</b> into your VisIt config directory:

    cp /path/to/visitconfigs/ctandxmlsbhdisks/bhbhdisk_pink.ct ~/.visit/

### Source the Parameters File
The following code is a default params file for the black hole with accretion disk
...
```
<code>params file for black hole with accretion disk</code>
{: style="text-align: center;"}

The `params` file contains important initial configuration:
<ul>
    <li>Which variables to plot: For Black Hole with Accretion Disk, make data equal to 'bht'</li>
...
![alt text](img/BHBH/bht_clip_max0.5_0031.png)
<div style="text-align: center;">
    <p>Black Hole with Accretion Disk plot</p>
</div>
